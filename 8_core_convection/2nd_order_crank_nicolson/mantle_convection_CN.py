...
#==============================================================================
pvd_velocity = \
    dlfn.File("CN_{0:1.1e}_{1:1.1e}_{2}s/solution_velocity.pvd".\
                         format(int(rayleigh), float(delta_t), int(t_end)))
pvd_pressure = \
    dlfn.File("CN_{0:1.1e}_{1:1.1e}_{2}s/solution_pressure.pvd".\
                         format(int(rayleigh), float(delta_t), int(t_end)))
pvd_temperature = \
    dlfn.File("CN_{0:1.1e}_{1:1.1e}_{2}s/solution_temperature.pvd".\
                         format(int(rayleigh), float(delta_t), int(t_end)))
#==============================================================================
# define nonlinear variational problem (CN scheme)
# momentum equation          
F_v = (inner(v, del_v) - inner(v0, del_v)) * dV \
    + dt * half * dlfn.sqrt(Pr/Ra) \
        * ( a_operator(v, del_v) + a_operator(v0, del_v) ) \
    + dt * half * ( c_operator(v, v, del_v) + c_operator(v0, v0, del_v) ) \
    - dt * b_operator(v, del_p) \
    - dt * b_operator(del_v, p) \
    - dt * half * (T * dot(e_r, del_v) - T0 * dot(e_r, del_v)) * dV
# energy equation
F_T = (dot(T, del_T) - dot(T0, del_T)) * dV \
    + dt * half / dlfn.sqrt(Ra*Pr) \
        * ( a_operator(T, del_T) - a_operator(T0, del_T)) \
    + dt * half * ( c_operator(v, T, del_T) - c_operator(v0, T0, del_T))
F_sum = F_v + F_T
